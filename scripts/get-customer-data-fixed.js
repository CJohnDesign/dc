 ...
 * Using the exact format specified in the API documentation
 * 
 * Usage:
 *   node scripts/get-customer-data-fixed.js
 */
...
async function main() {
  try {
    ...
    const loginResponse = await axios.post(API_URL, querystring.stringify(loginData), {
      headers: {
    ...
    // Call get_customer_data with the exact format from the documentation
    ...
    try {
      console.log('\nRequest payload:');
      console.log(JSON.stringify(customerDataRequest, null, 2));
      
      const customerDataResponse = await axios.post(API_URL, querystring.stringify(customerDataRequest), {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      });
      ...
      if (customerDataResponse.data && customerDataResponse.data.success) {
